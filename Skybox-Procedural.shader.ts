...
Properties {
	...
	_SunStrength ("Sun Strength", Range (10, -10.0)) = 1.0
	_Color ("Atmosphere Tint", Color) = (.5, .5, .5, 1)
	_GroundColor ("Ground", Color) = (.369, .349, .341, 1)
	_HdrExposure("HDR Exposure", Float) = 1.3
  _WaveScale ("Wave scale", Range (0.02,0.15)) = 0.07 // sliders
  _ReflDistort ("Reflection distort", Range (0.0, 1.5)) = 0.5
  _RefrDistort ("Refraction distort", Range (0,1.5)) = 0.4
  _RefrColor ("Refraction color", Color) = (.34, .85, .92, 1) // color
  _ReflectionTex ("Environment Reflection", 2D) = "default texture" {
    Filtereing Nearest
    Address Edge
    Mipmaps On
    Compare Gequal
  ...
  _BumpMap ("Bumpmap (RGB) ", 2D) = "dsdsdsdsdsds" { foo=bar}
}
...
Tags { "RenderType"="Opaque" "Queue"="Geometry"}
     Stencil {
            ...
            // Blend Off
Blend One Zero, SrcAlpha DstColor // 2x Multiplicative
            blendOp 1 sub, add
            // Blend Off
